...
class ParamsSelection():
    ...
    def brute_force(self, enc_type: str, func: Function, n: int):
            ...
            for batch in [2 ** i for i in range(4, 9)]:
                for size in range(dim // 2, dim, h_size):
                    for percent in np.arange(0.5, 1.0, h_percent):
                        ...
                        pred_data = normalizer.renormalize(
                            [model.predict(np.array(x).reshape(1, dim))[0] for x in norm_data])
                        cur_error = self.__compare(func, rand_data, pred_data)
                        if cur_error < error:
                            model.save(
                                '../../Saved models/Weights/' + f'{func.func_name}_brute_force_{enc_type}_{dim}_{size}.h5')
                            error = cur_error
                            hp_list.clear()
        ...
        with open('../../Saved models/Params/' + f'{func.func_name}_brute_force_{enc_type}_{dim}_{hp_list[2]}.txt',
                  'w') as f:
            f.write(
                f'func name: {func.func_name}\nepochs: {hp_list[0]}\nbatch: {hp_list[1]}\nencoded dim: {hp_list[2]}\nsample split: {hp_list[3]}')
